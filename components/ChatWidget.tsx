...
export const ChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'ai', content: "Hi! I'm Studify AI. I can see your uploaded files and help you study. Ask me anything!" }
  ]);
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50" ref={scrollRef}>
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`flex items-start max-w-[95%] gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  {/* Avatar for AI */}
                  {msg.role === 'ai' && (
                  ...
                  {msg.role === 'ai' && (
                    <TTSPlayer text={msg.content} simple={true} className="mt-1" />
                  )}
                </div>
                ...
                {msg.sources && msg.sources.length > 0 && (
                   <div className="mt-1 ml-9 text-xs text-slate-400 flex flex-wrap gap-1 max-w-[90%]">
                      {msg.sources.slice(0, 2).map((s, i) => (
                        <a key={i} href={s.uri} target="_blank" rel="noopener noreferrer" className="flex items-center hover:text-primary hover:underline bg-slate-100 px-1.5 py-0.5 rounded">
                          {s.title.substring(0, 15)}... <ExternalLink size={10} className="ml-1"/>
                        </a>
                      ))}
                      {msg.sources.length > 2 && <span className="bg-slate-100 px-1.5 py-0.5 rounded">+{msg.sources.length - 2} more</span>}
                   </div>
                )}
              </div>
            ))}
