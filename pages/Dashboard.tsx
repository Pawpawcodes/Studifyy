...
export const Dashboard: React.FC = () => {
  ...
  const ActionCard = ({ title, icon: Icon, color, to, desc }: any) => (
    <Link to={to} className="group bg-white p-6 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
      <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${color}`}>
        <Icon size={24} className="text-white" />
      </div>
      <h3 className="text-lg font-bold text-slate-800 mb-1 group-hover:text-primary transition-colors">{title}</h3>
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="md:col-span-2 bg-gradient-to-r from-primary to-blue-500 rounded-2xl p-6 text-white shadow-lg">
          ...
          <p className="opacity-90 mb-6">You're on a roll. Ready to crush your {user?.level} goals?</p>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="p-2 bg-orange-100 rounded-lg text-orange-600"><FileText size={20} /></div>
          </div>
          <div className="text-3xl font-bold text-slate-800">{flashcards.length}</div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex flex-col justify-between">
          ...
          <div className="text-3xl font-bold text-slate-800">{user?.studyHoursPerDay}h</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h3 className="font-bold text-slate-800 mb-4">Focus Areas (Weak Topics)</h3>
          <div className="flex flex-wrap gap-2">
            ...
            <button className="px-3 py-1 text-slate-400 text-sm hover:text-primary">+ Add more</button>
          </div>
        </div>
            ...
            <div className="space-y-3">
              {quizzes.slice(0, 3).map((q, i) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  <span className="text-slate-700">Completed Quiz: <b>{q.topic}</b></span>
                  <span className="text-slate-400">{q.completedAt ? new Date(q.completedAt).toLocaleDateString() : 'Just now'}</span>
                </div>
              ))}
            </div>
          )}
