...
import { Upload, FileText, Image as ImageIcon, Send, Loader2, Search, ExternalLink, Link as LinkIcon } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[calc(100vh-8rem)]">
      ...
      <div className="lg:col-span-1 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {files.map(f => (
            <div key={f.id} className="flex items-center p-3 border border-slate-100 rounded-lg hover:bg-slate-50">
              <div className="bg-blue-100 p-2 rounded text-blue-600 mr-3">
                {f.type === 'image' ? <ImageIcon size={16} /> : <FileText size={16} />}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">{f.name}</p>
                <p className="text-xs text-slate-400">{new Date(f.uploadDate).toLocaleDateString()}</p>
              </div>
            </div>
          ))}
      ...
      <div className="lg:col-span-2 flex flex-col h-full space-y-4">
        <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 p-6 overflow-y-auto">
          {!answer && !isSolving && (
            <div className="h-full flex flex-col items-center justify-center text-center opacity-60">
               ...
               <p className="text-sm text-slate-500 mt-2">I can read diagrams, handwriting, and text!</p>
            </div>
          )}
          ...
          {answer && (
            <div className="prose prose-slate max-w-none relative">
               <div className="flex justify-between items-center mb-4">
                  <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wide">AI Solution</h4>
              ...
              {sources.length > 0 && (
                <div className="mt-6 pt-4 border-t border-slate-100">
                  ...
                  <ul className="space-y-1">
                    {sources.map((s, i) => (
                      <li key={i}>
                        <a href={s.uri} target="_blank" rel="noreferrer" className="text-xs text-blue-500 hover:underline flex items-center">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200 flex items-center space-x-2">
          ...
          >
            {isSolving ? <Loader2 size={20} className="animate-spin" /> : <Send size={20} />}
          </button>
        </div>
      </div>
    </div>
