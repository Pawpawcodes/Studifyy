...
export const SettingsPage: React.FC = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8">
      <h2 className="text-2xl font-bold text-slate-800 mb-8">Settings & Profile</h2>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
           ...
           <div className="space-y-4">
             ...
             <div>
               <label className="block text-sm font-medium text-slate-500 mb-1">Education Level</label>
               <select 
                 className="w-full border border-slate-300 rounded-lg px-4 py-2 outline-none"
                 value={user.level}
                 onChange={(e) => updateUser({ level: e.target.value as EducationLevel })}
               >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
           ...
           <div className="flex items-center justify-between py-2">
             <div>
               <p className="font-medium text-slate-800">Auto-Play Text-to-Speech</p>
               <p className="text-sm text-slate-500">Automatically read aloud explanations and chat responses.</p>
             </div>
             <button 
               onClick={toggleAutoPlay}
               className={`w-12 h-6 rounded-full transition-colors relative ${user.autoPlayAudio ? 'bg-primary' : 'bg-slate-300'}`}
             >
               <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${user.autoPlayAudio ? 'left-7' : 'left-1'}`} />
             </button>
           </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 opacity-70">
           <h3 className="text-lg font-bold text-slate-700 mb-4">Academic Focus</h3>
           <div className="flex flex-wrap gap-2">
             {user.subjects.map(s => (
               <span key={s} className="px-3 py-1 bg-slate-100 rounded-full text-sm text-slate-600">{s}</span>
             ))}
           </div>
           <p className="text-xs text-slate-400 mt-4">To change subjects, please reset the app.</p>
        </div>
      </div>
    </div>
  );
