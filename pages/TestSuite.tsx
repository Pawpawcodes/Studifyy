    ...
    try {
      ...
      if (ttsResponse.url) {
        results.push({
          ...
          status: "pass",
          details: {
            latencyMs: Math.round(ttsEnd - ttsStart),
            url: ttsResponse.url,
          },
        });
      } else {
      ...
      const agentResponse = await orchestrateRequest(
        "Hello, do I have any weak topics?",
        user,
        files,
        "",
      );
      ...
      if (agentResponse.text) {
        results.push({
          ...
          status: "pass",
          details: {
            latencyMs: Math.round(agentEnd - agentStart),
        ...
        results.push({
          test: "Orchestrator Agent",
          status: "fail",
          details: { error: "No response text" },
        });
      }
      ...
      if (files.length > 0) {
        ...
        results.push({
          test: "File Context",
          status: "warn",
          details: { message: "No files uploaded to test RAG" },
        });
      }
    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      {!report && !running && (
        <div className="bg-white p-12 rounded-xl border border-dashed border-slate-300 text-center text-slate-500">
          Click "Run End-to-End Tests" to validate RAG, Agents, TTS, and Memory
          systems.
        </div>
      )}
      ...
      {report && (
        <div className="space-y-6">
          <div
            className={`p-4 rounded-lg border ${report.summary.includes("Crashed") ? "bg-red-50 border-red-200 text-red-800" : "bg-green-50 border-green-200 text-green-800"}`}
          >
              ...
              <tbody className="divide-y divide-slate-100">
                {report.results.map((res, i) => (
                  <tr key={i} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-4">
                      {res.status === "pass" && (
                        <span className="inline-flex items-center text-green-600 px-2 py-1 bg-green-50 rounded text-xs font-bold uppercase">
                          <CheckCircle size={14} className="mr-1" /> Pass
                        </span>
                      )}
                      {res.status === "fail" && (
                        <span className="inline-flex items-center text-red-600 px-2 py-1 bg-red-50 rounded text-xs font-bold uppercase">
                          <XCircle size={14} className="mr-1" /> Fail
                        </span>
                      )}
                      {res.status === "warn" && (
                        <span className="inline-flex items-center text-orange-600 px-2 py-1 bg-orange-50 rounded text-xs font-bold uppercase">
